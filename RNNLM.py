...
class RNNLM_Model(nn.Module):
  ...
  def add_embedding(self, input_x):
    ...
    """
    ### YOUR CODE HERE
    input_x = [self.embedding_layer(input_x[:, i]) for i in range(self.config.num_steps)]
    ### END YOUR CODE
    return input_x
  ...
  def add_model(self, input_x, initial_state):
    ...
    for inp in input_x:
      hH = torch.matmul(previous_state, self.H)
      eI = torch.matmul(inp, self.I)
      previous_state = torch.sigmoid(hH + eI + self.b1)
      rnn_outputs.append(self.output_drop(previous_state))
  ...
  def add_projection(self, rnn_outputs):
    ...
    for out in rnn_outputs:
      o = torch.matmul(out, self.U) + self.b2
      outputs.append(o)
    ### END YOUR CODE
    return outputs
...
def compute_loss(ouputs, y, criterion):
  ...
  """ 
  ### YOUR CODE HERE
  outputs = torch.stack(ouputs, dim=1)
  X = outputs.flatten(start_dim=0, end_dim=1)
  Y = y.flatten()
  loss = criterion(X, Y)
  ### END YOUR CODE
  return loss
...
def test_RNNLM():
  ...
  model_optimizer = torch.optim.SGD(params=our_model.parameters(), lr=config.lr, momentum=0, weight_decay=2e-4)
  ### END YOUR CODE
